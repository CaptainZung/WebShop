...
function Home(){
  ...
  const {getQtyWL} = useContext(UserContext)
  const AddToWL = (productId)=>{
    setWishList((prev)=>{
      const newWL = {...prev}
      newWL[productId] ? delete newWL[productId] : (newWL[productId] = 1)
      const total = ()=>{
        return Object.values(newWL).reduce((sum, value) => sum + value, 0)
      }
      getQtyWL(total())
    ...
    .then(res =>{
      ...
      const {getTongQty} = useContext(UserContext)
    const getIdProduct = (product) => {
      let idProduct = localStorage.getItem("cart")
      idProduct = idProduct ? JSON.parse(idProduct) : {}
      idProduct[product] = (idProduct[product] || 0) + 1
             const total =()=>{
      return Object.values(idProduct).reduce((sum, value)=> sum + value, 0) 
      }
      getTongQty(total())
      localStorage.setItem("cart",JSON.stringify(idProduct))
